...
contract AuditFuzzTest is Test {
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  (b)  Seller revokes approval or moves NFT → settle() reverts entirely.
    //      No stall state - the keeper bot retries on the next block once
    //      the transfer issue is resolved.
    // ════════════════════════════════════════════════════════════════════════
    ...
    function test_sellerRevokeCausesSettleRevert() public {
        ...
        // settle() reverts entirely because transferFrom fails (no approval).
        vm.expectRevert();
        ah.settle(id);

        // Auction remains unsettled; winner's escrow intact.
    ...
    function test_sellerMovedNftCausesSettleRevert() public {
        ...
        // settle() reverts entirely because transferFrom fails (seller no longer owns).
        vm.expectRevert();
        ah.settle(id);
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  (c)  Non-receiving bidder for an OfferBook REJECTED offer. (Audit-#3)
    //      Only the seller can reject/refund - no permissionless
    //      refundExpiredOffer function exists.
    // ════════════════════════════════════════════════════════════════════════
    ...
    function test_offerRejectedRefundPushFallback() public {
        ...
        // Seller rejects the offer - bidders cannot refund themselves.
    ...
    // ════════════════════════════════════════════════════════════════════════
    //  (f)  (removed: no stalled auctions in v10 - settle() reverts entirely
    //      on any transfer failure. The keeper bot retries.)
    // ════════════════════════════════════════════════════════════════════════
    ...
    /// @dev Seller-moved-NFT → settle() reverts entirely (no stall).
    ///      The keeper bot retries once the transfer issue is resolved.
    ...
    /// @dev (no settleUnstuck - removed in v10. All payout paths are in settle().)
    ...
    /// @dev (removed: no reclaim function - settle() reverts on transfer failure)
    ...
    ///      Uses the inherited `_pay()` from MarketplaceCore so the
    ///      push-fallback emits the PushFailed event.
    function test_offer_rejectOffer_emitsPushFailed() public {
        ...
        GreedyReceiver bidder = new GreedyReceiver();
        bidder.setBlocked(false); // for makeOffer escrow
        vm.deal(address(bidder), 10 ether);
    ...
    /// @dev OfferBook.rejectOffer with non-receiving bidder → PushFailed fires.
    ///      (duplicate name removed - handled by test_offer_rejectOffer_emitsPushFailed above)
    ...
    // ════════════════════════════════════════════════════════════════════════════
    //  (m)  R-01 regression test (v28 - Round 5):
    //
    ...
    ///      No funds are silently lost. Uses rejectOffer to create the
    ///      pendingReturns credit (refundExpiredOffer removed in v10).
    function test_withdrawRefundRestoreOnFailure() public {
        ...
        // Build a pendingReturns credit via OfferBook rejectOffer path.
        uint256 tid = nft.mint(seller);
        vm.prank(seller);
        nft.setApprovalForAll(address(ob), true);
        ...
        // Seller rejects the offer - push will fall back to pull.
        bidder.setBlocked(true);
        ...
        assertEq(
            ...
            1 ether,
            "GreedyReceiver parked in pendingReturns via rejectOffer push fallback"
        );
