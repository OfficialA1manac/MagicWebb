...
for(const f of files){
  ...
  if(!["deployed","read-only","not-deployed"].includes(d.status)) throw new Error(f+": bad status");
  // read-only: a real app + origin exist (network appears in switchers) but
  // there are no contracts yet — addresses must be null, app info must not be.
  if(d.status==="read-only"){
