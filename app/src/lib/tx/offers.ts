...
export const refundExpiredOffer = (a: { nft: Address; tokenId: bigint; bidder: Address }, hooks?: TxHooks) =>
  runTx({ title: 'Refund expired offer', request: async () => buildRefundExpiredOffer(a.nft, a.tokenId, a.bidder), summary: [['What happens', 'The expired offer’s full amount goes back to the bidder’s wallet'], ['Cost', 'Gas only']], success: { message: 'Expired offer refunded to the bidder', action: { label: 'See offers', href: '/offers' } } }, hooks);

export const setOfferEligible = (a: { nft: Address; eligible: boolean; name?: string }, hooks?: TxHooks) =>
  runTx({ title: `${a.eligible ? 'Enable' : 'Disable'} offers for ${a.name ?? 'collection'}`, request: async () => buildSetOfferEligible(a.nft, a.eligible), summary: [['What it does', a.eligible ? 'Buyers can make offers on every NFT in this collection' : 'Buyers can no longer make offers on this collection'], ['Who can do this', 'Only the collection owner']], success: { message: a.eligible ? 'Offers enabled for this collection' : 'Offers disabled for this collection', action: { label: 'View collection', href: `/collection/${a.nft}` } } }, hooks);
