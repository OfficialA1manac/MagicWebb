<script lang="ts">
  ...
  function quickBuy(e: MouseEvent) {
    ...
    if (!mw) { location.href = '/token/' + item.collection + '/' + item.token_id; return; }
    mw.buy({ nft: item.collection, tokenId: item.token_id, seller: item.seller, priceWei: item.price_wei, name: item.name || '#' + item.token_id }).catch(() => {});
  }
...
<style>
  ...
  .buy-btn {
    border: 0; cursor: pointer; font-family: inherit; min-height: 32px;
